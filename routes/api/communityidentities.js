...
router.get('/', async function(req, res) {
    ...
    const rows = await models.CommunityIdentity.findAll();
    res.json(rows);
});
...
router.post('/', async function(req, res) {
    ...
    const row = models.CommunityIdentity.build(req.body);
  ...
  router.get('/:id', async function(req, res) {
    const row = await models.CommunityIdentity.findByPk(req.params.id);
    if (row) {
      res.json(row);
    } else {
