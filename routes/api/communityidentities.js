...
router.get('/', async function(req, res) {
    ...
    const rows = await models.CommunityIdentities.findAll();
    res.json(rows);
});
...
router.post('/', async function(req, res) {
    ...
    const row = models.CommunityIdentities.build(req.body);
  ...
  router.get('/:id', async function(req, res) {
    const row = await models.CommunityIdentities.findByPk(req.params.id);
    if (row) {
      res.json(row);
    } else {
