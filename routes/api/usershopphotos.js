...
router.get('/', async function(req, res) {
    ...
    const rows = await models.UserShopPhotos.findAll();
    res.json(rows);
});
...
router.post('/', async function(req, res) {
    ...
    const row = models.UserShopPhotos.build(req.body);
  ...
  router.get('/:id', async function(req, res) {
    const row = await models.UserShopPhotos.findByPk(req.params.id);
    if (row) {
      res.json(row);
    } else {
