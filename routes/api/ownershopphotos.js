...
router.get('/', async function (req, res) {
  ...
  const rows = await models.OwnerShopPhoto.findAll(options);
  res.json(rows);
});
...
router.post('/', async function (req, res) {
  ...
  const row = models.OwnerShopPhoto.build(req.body);
...
router.get('/:id', async function (req, res) {
  const row = await models.OwnerShopPhoto.findByPk(req.params.id);
  if (row) {
    res.json(row);
  } else {
