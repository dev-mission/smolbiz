...
router.get('/', async function(req, res) {
    ...
    const rows = await models.OwnerShopPhotos.findAll();
    res.json(rows);
});
...
router.post('/', async function(req, res) {
    ...
    const row = models.OwnerShopPhotos.build(req.body);
  ...
  router.get('/:id', async function(req, res) {
    const row = await models.OwnerShopPhotos.findByPk(req.params.id);
    if (row) {
      res.json(row);
    } else {
